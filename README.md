...
## 9. Assignment Rubric Mapping (M1-M5)
...
- FastAPI inference service with required endpoints (`/health`, `/predict`).
- Dependencies defined and pinned in `requirements.txt`.
- Dockerized service via `Dockerfile`.

### M3: CI for Build/Test/Image
- Unit tests for preprocessing/data loading (`tests/test_dataset.py`) and model/API.
- GitHub Actions CI pipeline in `.github/workflows/ci.yml`.

### M4: CD Pipeline & Deployment
- Kubernetes manifests: `k8s/deployment.yaml`, `k8s/service.yaml`.
- Manual CD workflow in `.github/workflows/cd.yaml` for registry publish and deploy.
- Smoke test script used in CI and post-deploy.

### M5: Monitoring, Logs & Submission
- Basic API logging and latency/request metrics in service.
- README + project artifacts consolidated for submission.
